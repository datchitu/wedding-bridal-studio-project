...
const HistoryPaymentDetail = (props) => {
        ...
        if (formIsValid === true) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const response = await updatePerformDate(transactionUser.id, transactionUser.performDatetime);
                    const promise = Promise.resolve(response);
                    promise.then(data => {
                    // console.log(data);
                    if (data.code == 200) {
                        paymentTransaction(transactionId, note);
                        Swal.fire({
                            position: 'top-end',
                            icon: 'success',
    ...
    //     try {
    //         let response = await voucherApi.getVoucherById(id);
    ...
    //    console.log(">>> check user: ", userId);
       console.log(">>> check transaction user: ", transactionUser);
...
{/* 
                            <button
                                   type="button" className="btn-primary" onClick={ handleCheck}>Kiểm tra thông tin thanh toán</button> */}
